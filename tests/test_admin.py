...
def test_admin_edit_account():
    with app.test_client() as client:
        ...
        response = client.post('/edit_account?id={}'.format(str(acc['_id'])), data=dict(username='5@gmail.com', investments=str(acc['investments']), stocks=str(acc['stocks']), saved=str([]), balance='123', submit='Save'))
